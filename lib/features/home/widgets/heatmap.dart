...
/// 'YYYY-MM-DD' → 'Mar 12'; falls back to the raw string if unparsable.
String _bestDayLabel(String isoDate) {
  final d = DateTime.tryParse(isoDate);
  if (d == null) return isoDate;
  return '${_monthAbbr[d.month - 1]} ${d.day}';
}

/// 0 = no break time; then one level per 15 minutes:
/// 1-15m, 16-30m, 31-45m, 46m+.
int _levelFor(int minutes) {
  if (minutes == 0) return 0;
  final level = (minutes / 15).ceil();
...
class _HeatmapCardState extends State<HeatmapCard> {
  ...
  Widget build(BuildContext context) {
    final totals = widget.repository.yearDailyMinutes(_year);
    ...
    for (var i = 0; i < dayCount; i++) {
      final d = yearStart.add(Duration(days: i));
      final col = i ~/ 7;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: Text(
              _selectedDay == null
                  ? ' '
                  : '${_monthAbbr[_selectedDay!.month - 1]} ${_selectedDay!.day} · ${formatMinutes(_selectedValue ?? 0)}',
              style:
            ...
            children: [
              ...
              Expanded(
                  child: _Callout(
                      ...
                      value: best == null
                          ? 'None yet'
                          : '${formatMinutes(best.minutes)} · ${_bestDayLabel(best.date)}')),
              const SizedBox(width: 10),
              Expanded(
                  child: _Callout(
                      label: 'Year total',
                      value: formatMinutes(yearTotal))),
            ],
          ),
        ],
      ),
