...
/// Calendar-based day arithmetic; Duration-based day math drifts across DST.
DateTime _daysFrom(DateTime d, int days) =>
    DateTime(d.year, d.month, d.day + days);

/// Ported 1:1 from the original app's `_compute_trend` (squat_reminder.pyw).
TrendResult computeTrend(SquatRepository repo, TrendMode mode, int offset) {
  final today = DateTime.now();

  if (mode == TrendMode.week) {
    final end = _daysFrom(today, -7 * offset);
    final start = _daysFrom(end, -6);
    final dates = List.generate(7, (i) => _isoDate(_daysFrom(start, i)));
    final label =
        '${_monthAbbr[start.month - 1]} ${start.day} – ${_monthAbbr[end.month - 1]} ${end.day}';
    final totals = repo.dailyTotals(dates.first, _isoDate(_daysFrom(end, 1)));
  ...
  if (mode == TrendMode.month) {
    ...
    final nextMonthStart = _isoDate(DateTime(y, m + 1, 1));
    final totals = repo.dailyTotals(dates.first, nextMonthStart);
    final values = dates.map((d) => totals[d] ?? 0).toList();
    return TrendResult(label: label, dates: dates, values: values);
  }
  ...
  final values = repo.monthlyTotals(y);
...
class _TrendChartState extends State<TrendChart> {
  ...
  Widget build(BuildContext context) {
    ...
    String? caption;
    if (selected != null) {
      final v = _data.values[selected];
      if (_mode == TrendMode.year) {
        caption = '${_monthNames[selected]} · $v squats';
      } else {
        final d = DateTime.parse(_data.dates[selected]);
        final isToday = isCurrent && selected == _data.values.length - 1;
        caption =
            '${isToday ? "Today" : "${_monthAbbr[d.month - 1]} ${d.day}"} · $v squats';
      }
    }
